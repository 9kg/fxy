...
// 获取当前ip
const cur_ip = (os => {
    let _cur_ip = '127.0.0.1',en = os.networkInterfaces().en0 || os.networkInterfaces().en1;
    en.forEach(item => {
        item.family === 'IPv4' && (_cur_ip = item.address);
    });
    return _cur_ip;
})(require('os'));
// 配置对象
const cfg = (() => {
    ...
    return {
        ...
        less_to: _dev + '/css',
        concat_src: [_src + 'js/zepto.js', _src + 'js/swiper.js', _src + 'js/base.js'],
        concat_to: _dev + '/js',
        htmlmin_src: [_dev + '*.html'],
        htmlmin_to: _build,
        copy2dev: [_src + 'html/**/*.html',
                    _src + 'font/**/*.{eot,ttf}',
                    _src + 'css/**/*.css',
                    _src + 'json/**/*.json',
                    _src + 'img/**/*.{png,jpg,gif,ico}',
                    _src + '*.{html,js,css,json,png,jpg,gif,ico}',
                    '!' + _src + 'package.json',
                    '!' + _src + 'gulpfile.js'],
        copy2build: [_dev + 'font/**/*.{eot,ttf}'],
...
gulp.task('less', () => {
    ...
    .pipe(less())
    .pipe(prefix({
        browsers: ['Android >= 4', 'Chrome >= 40', 'last 6 Firefox versions', 'iOS >= 6', 'Safari >= 6']
    }))
    .pipe(md5_suffix())
...
gulp.task('concat',function(){
    return gulp.src(cfg.concat_src)
    .pipe(maps.init())
    .pipe(concat('base.js'))
